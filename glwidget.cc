...
bool GLWidget::LoadModel(const QString &filename) {
  ...
  if (res) {
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(float)*mesh_->vertices_.size(), &mesh_->vertices_[0],
    GL_STATIC_DRAW);
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, 0);
    glEnableVertexAttribArray(0);
    //--- NORMALE ---
    glBindBuffer(GL_ARRAY_BUFFER, VBO_n);
    glBufferData(GL_ARRAY_BUFFER, sizeof(float)*mesh_->normals_.size(), &mesh_->normals_[0],
                 GL_STATIC_DRAW);
    glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 0, 0);
    glEnableVertexAttribArray(1);
    //--- COORDINATE ---
    glBindBuffer(GL_ARRAY_BUFFER, VBO_tc);
    glBufferData(GL_ARRAY_BUFFER, sizeof(float)*mesh_->texCoords_.size(), &mesh_->texCoords_[0],
                 GL_STATIC_DRAW);
    glVertexAttribPointer(2, 2, GL_FLOAT, GL_FALSE, 0, 0);
    glEnableVertexAttribArray(2);
    //--- FACES/ INDICI ---
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, VBO_i);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(unsigned int)*mesh_->faces_.size(), &mesh_->faces_[0],
                 GL_STATIC_DRAW);
    //--- CHIUSURA del VAO---
    glBindVertexArray(0);
    ...
    //faces -> elements
    // 1. Definiamo gli 8 vertici del cubo (X, Y, Z)
    skyVertices_ = {
        // Base
    ...
    // 2. Definiamo i 36 indici per formare i 12 triangoli
    skyFaces_ = {
        // Faccia Anteriore
        2, 3, 7,   2, 7, 6,
        // Faccia Posteriore
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(float)*skyVertices_.size(), &skyVertices_[0],
                 GL_STATIC_DRAW);
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, 0);
    glEnableVertexAttribArray(0);
    ...
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(unsigned int)*skyFaces_.size(), &skyFaces_[0],
                 GL_STATIC_DRAW);
...
bool GLWidget::LoadRoughnessMap(const QString &filename)
{
    ...
    glBindTexture(GL_TEXTURE_2D, 0);    //TODO END
    update();
    return res;
}
    ...
    if (initialized_) {
        ...
        if (mesh_ != nullptr) {
            ...
            if(skyVisible_) {
                ...
                glDisable(GL_CULL_FACE); // Spegne l'ottimizzazione

                glBindVertexArray(VAO_sky);
                glDrawElements(GL_TRIANGLES, skyFaces_.size(), GL_UNSIGNED_INT,(GLvoid*) 0);
                glBindVertexArray(0);

                glEnable(GL_CULL_FACE);  // La riaccende per non rovinare la sfera
                // TODO END.
            }
        }
    }
